...
/**
 * CheckoutV1 - Current Flow
 * - Quantity selector and upsells in Step 1
 * - Collapsed order summary (default)
 * - Multi-step checkout
 */
export default function CheckoutV1({
  ...
  return (
    <div ref={formRef} className="max-w-full overflow-hidden">
      ...
      {/* Step 1: Contact + Quantity + Upsells */}
      {currentStep === 1 && (
        <>
          <SocialProof />

          {/* Quantity Selector & Upsells */}
          <div className="bg-white border border-gray-200 rounded-lg p-4 mb-3 shadow-sm">
            <QuantitySelector
              quantity={cartItems[0].quantity}
              onUpdateQuantity={(qty) => updateQuantity(cartItems[0].id, qty)}
            />
          </div>
